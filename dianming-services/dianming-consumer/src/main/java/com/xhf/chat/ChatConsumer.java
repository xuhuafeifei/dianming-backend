package com.xhf.chat;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.rabbitmq.client.Channel;
import com.xhf.apis.ChatFeignService;
import com.xhf.common.constant.ChatMQConstants;
...
public class ChatConsumer {
    ...
    public void receiveMessage(Message message, Channel channel) throws InterruptedException, IOException {
        ...
        try {
            // Thread.sleep(1000);
            log.info("msg: {}", msg);
            R flag = chatFeignService.synchronization(msg);
            log.info("同步结果 : {}", flag);
            // 消息同步异常, mq消息回退
            if (0 != Integer.parseInt((String) flag.get("code"))) {
            ...
            // 修改mq_message_info信息
            R r = chatFeignService.updateMessageInfo(mqMsgId, "", true);
            log.info("mq_message_info 修改结果:{}", r);
        } catch (Exception e) {
            e.printStackTrace();
            // 消息确认, 并重新入队
            channel.basicReject(deliveryTag, true);
            chatFeignService.updateMessageInfo(mqMsgId, e.getMessage(), false);
    ...
    public void ChatWarningHandler(Message message, Channel channel) {
        ...
        try {
            ...
            // 存储警告信息, 修改存储MQ的发布信息
            String json1 = objectMapper.writeValueAsString(waringInfo);
            String json2 = objectMapper.writeValueAsString(messageInfo);
            HashMap<String, String> params = new HashMap<>();
            params.put("waringInfo", json1);
            params.put("messageInfo", json2);
            chatFeignService.waringQueueHandler(params);
